...
excel_file = 'Vehicle information_20250110_102007.xls' #Paste the DMS output file name
...
wb_name = 'result.xlsx' #continue file to process
...
ws_process = ws_sorted[
    (~ws_sorted["VIN"].str.upper().str.startswith("TEST", na=False))
    & (~ws_sorted.iloc[:, 2].isna())  # 第三列不是缺失值
    & (ws_sorted.iloc[:, 2] != "")
]  # 第三列不是空字符串]
with pd.ExcelWriter(wb_name, engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:
...
for row in range(2,max_row+1):
    ...
    ws_move_col.cell(row=row, column=5).value = ws_move_col.cell(row=row, column=5).value.strftime("%d/%m/%Y") 
